...
| [`market-research-sheet/`](./market-research-sheet/) | A Google Apps Script that turns a Sheet into a market-research worksheet: describe a requirement, get similar open/closed requirements, award evidence, and a ranked vendor table. Copy-paste install, no server. |
| [`market-research-memo/`](./market-research-memo/) | The docs themselves: find similar requirements, download their actual SOWs/PWSs/RFP packages into Drive, match each to the award it produced, and draft the FAR Part 10-style memo as a Google Doc that cites every exhibit. || [`saved-search-watcher/`](./saved-search-watcher/) | A YAML-driven watcher: pull a saved search on a schedule, diff against a JSON state file, alert on what's new. |
