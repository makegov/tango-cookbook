...
- Document downloads hit SAM.gov's public attachment URLs directly — no API key on those requests. Amendments repost attachments, so the manifest is deduped by filename first (trying each posted copy until one downloads). Every download is individually fault-tolerant: a file that fails is recorded in the index and memo (`not retrieved (HTTP 404)`) and the run continues.
- Winners come from `GET /api/contracts/?solicitation_identifier=…` for every scanned *closed* requirement (capped at 25 beyond the doc pulls — open notices can't have awards yet). If the exact number misses, it retries with punctuation stripped — FPDS drops the hyphens SAM keeps (the same trick as [`../tango-lookup-extension/`](../tango-lookup-extension/)) — and then by **PIID**, because award notices put the award number, not the solicitation number, in SAM's solicitation field. Junk identifiers under six characters are skipped rather than matched to noise, and duplicate matches (the same award reached through presol + sol notice records) are deduped by award key.
