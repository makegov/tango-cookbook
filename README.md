...
| [`market-research-memo`](./examples/market-research-memo/) | The docs themselves: pull similar requirements' actual SOWs and RFP packages into Drive, match winners, and draft the market research memo as a Google Doc. || [`saved-search-watcher`](./examples/saved-search-watcher/) | A YAML-driven watcher: poll a saved search on a schedule, diff against a JSON state file, alert on what's new. |
